...
$(document).ready(function(){ 
	...
	function init_template(){	
		...
		/*$('.demo-light').on('click', function(){
			...
			$('.menu-sidebar').removeClass('menu-dark');gallery-filter
		...
		*/
		...
		$('.menu-sidebar').addClass('inactive-touch');
		$('.menu-items a').append('<i class="la la-angle-right"></div>');
		$('.page-list-items li a.flag.active').append('<i class="la la-check"></div>');
		...
		$(".back-button-comic").on('click', function(){
			$('#page-transitions').addClass('back-button-clicked');
			$('#page-transitions').removeClass('back-button-not-clicked');
		...
		// close play episode btn
		$('.back-button-play').on('click', function(){
			$('#page-transitions').addClass('back-button-clicked');
			$('#page-transitions').removeClass('back-button-not-clicked');
			var sid = window.localStorage.getItem("story");
			window.location.replace("story.html#"+sid);
			//window.history.go(-1);
        ...
        jQuery(document).ready(function(e) {
            function t(t, n) {
				...
				var regToken = localStorage.getItem('RegToken');
				console.log('registration-token',regToken);
				params['registration_token'] = regToken;
				...
				$.ajax({
					  ...
					  success: function (response) {
						   ...
						   if(response.code =="activationreg"){
						   ...
						   $('#toast-2').addClass('show-toast');
						    setTimeout(function(){window.location.replace("dummy-page-posting.html");},1500);
						   }
            ...
            function n(n, r) {
                ...
                e("#" + n + " .requiredField").each(function(i) {
                    ...
                    if (loginFormSubmitted == "false" && i == e("#" + n + " .requiredField").length - 1) {
                        t(n, r)
                    }
                })
            }
				...
				$.ajax({
					  ...
					  success: function (response) {
						   ...
						   if(response=="Sucess"){
							  ...
							  $('#toast-3').addClass('show-toast');
							  setTimeout(function(){$('#toast-3').removeClass('show-toast');},2000);  
						   }
        ...
        jQuery(document).ready(function(e) {
            ...
            function n(n, r) {
                ...
                e("#" + n + " .requiredField").each(function(i) {
                    ...
                    if (forgetFormSubmitted == "false" && i == e("#" + n + " .requiredField").length - 1) {
                        t(n, r)
                    }
                })
            }
            ...
            e("#forgetSubmitButton").click(function() {
                n(e(this).attr("data-formId"));
                return false
            })
        })
		...
		var signupFormSubmitted = "false";
        jQuery(document).ready(function(e) {
            function t(t, n) {
				if($("#passwordField").val() !=$("#vpasswordField").val()){
					    $("#vpasswordField").focus();
                        $("#vpasswordFieldError2").fadeIn(300);
                        return false
				}
				...
				$.ajax({
					  ...
					  success: function (response) {
						    ...
						    if(response.status == "Error"){
							   ...
							   if(langid==2){
								   if(response.msg =="eae"){
									 errMsg = "이미 등록된 이메일입니다. ";  
								   }
								   if(response.msg =="uae"){
									 errMsg = "이미 등록된 사용자명입니다.";  
								   }
							   }
                               ...
                               $("#formError3").html('<p class="center-text uppercase small-text color-white">'+errMsg+'</p>');
							   $("#usernameField").focus();
							   $("#usernameField").addClass("fieldHasError");
							   $("#formError3").fadeIn(300);
							   $(".loading-gif").addClass("hideit");
               ...
               /* e.post(e("#" + t).attr("action"), r, function(n) {
					...
					resp = JSON.parse(n);
					  resp.forEach(function (entry) {
						items.push(entry);
						//$$('#itemstoload').append(' <div class="card demo-card-header-pic"> <div style="background-image:url(' + entry.thumburl[0] + ')" class="card-header align-items-flex-end">' + entry.post_title + '</div> <div class="card-content card-content-padding"> <p class="date">Posted on January 21, 2015</p> <p>Quisque eget vestibulum nulla. Quisque quis dui quis ex ultricies efficitur vitae non felis. Phasellus quis nibh hendrerit...</p> </div> <div class="card-footer"><a href="#" class="link">Like</a><a href="#" class="link">Read more</a></div> </div>');
					  });
					  console.log('items');
					  console.log(items);
					//e("#" + t).hide();
            ...
            function n(n, r) {
                ...
                e("#" + n + " .requiredField").each(function(i) {
                    ...
                    if (signupFormSubmitted == "false" && i == e("#" + n + " .requiredField").length - 1) {
                        t(n, r)
                    }
                })
            }
...
if($('.unread-message').length){
	$('.unread-message').text(messageCounter());
}

$(document).on('click','.change-passcode',function(){
	var pass = $('.privacy-passcode').val();
	if((pass).length < 4){
		$('#toast-invalid').addClass('show-toast');
		return false;
	}else{
		var post_id = $('#my_post_id').val();
		$.ajax({
			url:'http://blmani.com/wp-json/aniparti/passcode_update_ex',
			method:'post',
			dataType:'json',
			data:{passcode:pass,post_id:post_id},
			beforeSend:function(){
			...
			success:function(resp){
				...
				if(resp.status == 'success'){
						$('#reset-passcode-modal').removeClass('active-flyin');
						$('#menu-hider').removeClass('menu-hider-active');
						$('.privacy-passcode').val('');
				}else{
					$('#toast-invalid').addClass('show-toast');
